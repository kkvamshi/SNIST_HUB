...
if($y!=0)
$sql = "SELECT * from academics where privacy%pow(10,$y)>=pow(10,$y-1) and privacy%pow(10,$b)>=pow(10,$b-1) order by time desc";
else
$sql = "SELECT * from academics order by time desc";
}
else{
	$pro_id=$_GET['pro_id'];
if($y!=0)
$sql = "SELECT * from academics where privacy%pow(10,$y)>=pow(10,$y-1) and privacy%pow(10,$b)>=pow(10,$b-1) and posted_by='$pro_id' order by time desc";
else
$sql = "SELECT * from academics where posted_by='$pro_id' order by time desc";
}
$res = $conn->query($sql);
$r=$res->num_rows;
$page = 1;
...
if ($result->num_rows > 0) {
	...
	while($row = $result->fetch_assoc())
	{
		$ploc=$row["post_loc"];
		$pid=$row["post_id"];
        $sql1 = "SELECT * from a_likes where post_id='$pid'";
        $res1 = $conn->query($sql1);
        $l=$res1->num_rows;
        $sql3 = "SELECT * from a_comments where post_id='$pid' order by time desc";
        $res3 = $conn->query($sql3);
        $c=$res3->num_rows;
		$post="";
		$post.='<div class="post_div" style="border-left:5px ridge #22dd22; background-color:#ffffff; padding-left:2px; margin-bottom:20px; border-radius:6px;" id='.$row["post_id"].'><div class="post_head">
...
<ul class="mdl-menu mdl-menu--bottom-right mdl-js-menu mdl-js-ripple-effect"
    for="'.$row["post_id"].'-menu">';
	    if($row['posted_by']==$id || $id=='admin'){
			...
			$post.='<li class="mdl-menu__item" onclick=report("'.$pid.'","academics") ><span>report</span></li>';
		}
		if($id=='admin')
			$post.='<li class="mdl-menu__item" onclick=report("'.$pid.'","academics") ><span>report</span></li>';
			$post.='<li class="mdl-menu__item" onclick=view("'.$pid.'","academics") ><span>view</span></li>';
...
<i class="material-icons" style="'; 
                $sql2 = "SELECT * from a_likes where post_id='$pid' and liked_by='$id'";
