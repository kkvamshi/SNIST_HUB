...
$sql="select * from staffposts where post_id='$pid'";
...
if ($result->num_rows > 0) {
	...
	while($row = $result->fetch_assoc())
	{
		...
		foreach($branch as $yr){
			echo "($yr)";
		}
		echo "</p></div>";
		$y=intval($year);
		if(bcmod($privacy,pow(10,$b))>=pow(10,$b-1))||$b==0){
$ploc=$row["post_loc"];
		$pid=$row["post_id"];
        $sql1 = "SELECT * from sp_likes where post_id='$pid'";
        $res1 = $conn->query($sql1);
        $l=$res1->num_rows;
        $sql3 = "SELECT * from sp_comments where post_id='$pid' order by time desc";
        $res3 = $conn->query($sql3);
        $c=$res3->num_rows;
		$post="";
		$post.='<div class="post_div" style="border-left:5px ridge black; background-color:#ffffff; padding-left:2px; margin-bottom:20px; border-radius:6px;" id='.$row["post_id"].'><div class="post_head">
...
<ul class="mdl-menu mdl-menu--bottom-right mdl-js-menu mdl-js-ripple-effect"
    for="'.$row["post_id"].'-menu">';
	    if($row['posted_by']==$id || $id=='admin'){
			...
			$post.='<li class="mdl-menu__item" onclick=report("'.$pid.'","staff") ><span>report</span></li>';
		}
		if($id=='admin')
			$post.='<li class="mdl-menu__item" onclick=report("'.$pid.'","staff") ><span>report</span></li>';
			$post.='<li class="mdl-menu__item" onclick=view("'.$pid.'","staff") ><span>view</span></li>';
...
<i class="material-icons" style="'; 
		...
		$post.='>thumb_up</i></button>
		     <input type="text" class="com" placeholder="Type your comment here" />
			  <button style="background-color:white;" class="comment mdl-button mdl-js-button mdl-button--icon" style="float:right;"  onclick=commented("'.$pid.'","'.$row["posted_by"].'")  height="100%">
  <i class="material-icons">comment</i>
</button>
			  </div>
			  <div class="lcs"><span class="likes">'.$l.' likes</span><span class="comments">'.$c.' comments</span></div>';
			 
				  $post.='<div class="post_comment" style="max-height:1200px;">';
			 if($c>0){
