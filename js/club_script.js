	...
	function liked(pid,pbyid){
		$(document).ready(function(){
		    $.ajax({
			url: 'Clike.php?pid='+pid+'&pbyid='+pbyid,
	...
	function view(pid,tab){
			$(document).ready(function(){
		 window.location="view_club_post.php?pid="+pid;
		});
	}
		function save(pid){
		...
		$(document).ready(function(){
		    $.ajax({
			url: 'Cedit.php?pid='+pid+'&content='+content,
			success: function(data){
	...
	var pro_id =   $('#proid').val();
		    $.ajax({
			url: 'Cdelete.php?pid='+pid+'&loc='+location,
			success: function(data){
				...
				var posts = parseInt($(".posts:first").val());
				if(pro_id.length>3)
				getresult('getclubs.php?page='+pagenum+'&posts='+posts+'&pp=1&pro_id='+pro_id);
			    else
				getresult('getclubs.php?page='+pagenum+'&posts='+posts+'&pp=1');
				$('#'+pid).html('Your post is deleted');
			}
			},
			error: function(){} 	        
	...
	function selbranch(){
		...
		$(document).ready(function(){
			...
			$('#'+pid+' .com').val("");
		    $.ajax({
			url: 'Ccomment.php?pid='+pid+'&com='+com+'&pbyid='+pbyid,
			success: function(data){
				          $('#'+pid+' .comments').html(data);
			     $.ajax({
			              url: 'Ccomments.php?pid='+pid,
			              success: function(dat){
	...
	var pro_id =   $('#proid').val();
		if(f==1){
		  if ($(window).scrollTop() > 650*($(document).height()/670-1)){
				...
				var posts = parseInt($(".posts:first").val());
				if(pro_id.length>3)
				getresult('getclubs.php?page='+pagenum+'&posts='+posts+'&pro_id='+pro_id);
			    else
				getresult('getclubs.php?page='+pagenum+'&posts='+posts);
			}
		  }
		}
	});
