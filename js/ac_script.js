	...
	function liked(pid,pbyid){
		$(document).ready(function(){
		    $.ajax({
			url: 'Alike.php?pid='+pid+'&pbyid='+pbyid,
		...
		function view(pid,tab){
			$(document).ready(function(){
		 window.location="view_ac_post.php?pid="+pid;
		});
	}
		function save(pid){
	...
	var pro_id =   $('#proid').val();
		    $.ajax({
			url: 'Adelete.php?pid='+pid+'&loc='+location,
			success: function(data){
				...
				var posts = parseInt($(".posts:first").val());
				if(pro_id.length>3)
				getresult('getacademics.php?page='+pagenum+'&posts='+posts+'&pp=1&pro_id='+pro_id);
			    else
				getresult('getacademics.php?page='+pagenum+'&posts='+posts+'&pp=1');
				$('#'+pid).html('Your post is deleted');
			}
			},
			error: function(){} 	        
	...
	function selbranch(){
		...
		$(document).ready(function(){
			...
			$('#'+pid+' .com').val("");
		    $.ajax({
			url: 'Acomment.php?pid='+pid+'&com='+com+'&pbyid='+pbyid,
			success: function(data){
				          $('#'+pid+' .comments').html(data);
			     $.ajax({
			              url: 'Acomments.php?pid='+pid,
			              success: function(dat){
	...
	$(window).scroll(function(){
		if(f==1){
		  if ($(window).scrollTop() > 650*($(document).height()/670-1)){
				...
				var posts = parseInt($(".posts:first").val());
				if(pro_id.length>3)
				getresult('getacademics.php?page='+pagenum+'&posts='+posts+'&pro_id='+pro_id);
			    else
				getresult('getacademics.php?page='+pagenum+'&posts='+posts);
