...
if($y!=0)
$sql = "SELECT * from general where privacy%pow(10,$y)>=pow(10,$y-1) and privacy%pow(10,$b)>=pow(10,$b-1) order by time desc";
else
$sql = "SELECT * from general order by time desc";
}
else{
	$pro_id=$_GET['pro_id'];
if($y!=0)
$sql = "SELECT * from general where privacy%pow(10,$y)>=pow(10,$y-1) and privacy%pow(10,$b)>=pow(10,$b-1) and posted_by='$pro_id' order by time desc";
else
$sql = "SELECT * from general where posted_by='$pro_id' order by time desc";
}
$res = $conn->query($sql);
$r=$res->num_rows;
$page = 1;
...
</div><div class="lcs"><span class="likes">'.$l.' likes</span><span class="comments">'.$c.' comments</span></div>';
